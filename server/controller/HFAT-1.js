...
import { HFAT1, HFAT1_FINAL,HFAT1_Mapping } from "../Database/HFAT-1.js";
...
export const HFAT1FINALGet = async (req, res, next) => {
  try {
    const adminId = req.user.id;
    const state = req.user.sitename;
    const role = req.user.role;
    ...
    var HFAT1Data;
    if (role === "superadmin") {
      HFAT1Data = await HFAT1_FINAL.find();
    } else {
      HFAT1Data = await HFAT1_FINAL.find({ uniqueCode: { $regex: regex } });
    }
...
export const HFAT1AndAMBULANCEFINALGet = async (req, res, next) => {
  try {
    const id = req.user.id;
    const state = req.user.sitename;
    const role = req.user.role;
    ...
    // Apply conditional logic based on the role
    if (role === "superadmin") {
      HEAT1Data = await HFAT1_FINAL.aggregate([
        ...lookupAmbulanceDetails, // Reuse lookup logic for superadmin
      ]);
    } else {
      HEAT1Data = await HFAT1_FINAL.aggregate([
...
export const HFAT1FinalUpdateController = async (req, res) => {
  try {
    // const { id } = req.params;
    const data = req.body;
    const updatedData = await HFAT1_FINAL.findByIdAndUpdate(data._id, data);
    res.status(200).json({updatedData,succes:true});
  }
  catch (error) {
    console.error(error);
...
// HAFT1UploadController
export const HAFT1UploadControllerold = async (req, res) => {
 try {
    if (!req.file) {
    ...
    await fs.writeFileSync(newFilePath, file.buffer);
    ...
    for await (const row of stream) {
      ...
      if (batch.length === batchSize) {
        await HFAT1_FINAL.insertMany(batch);
        batch = [];
      }
    }
    ...
    fs.unlink(tempFilePath, (err) => {
      if (err) {
        ...
        // optionally handle the error, but still send success response
      }
    });

    res.status(200).send(`File uploaded and data imported successfully: ${newFilePath}`);

    // res.status(200).send(`File saved successfully at ${newFilePath}`);
  } catch (error) {
...
export const HAFT1UploadControllerOLD = async (req, res) => {
  try {
    ...
    const mappings = await HFAT1_Mapping.find({}).lean();
    const headerMapping = {};
    mappings.forEach((m) => {
      if (m.headerName && m.field) headerMapping[m.headerName.trim()] = m.field.trim();
    });
    console.log("HFAT-1 Header Mapping Loaded:", headerMapping.length);
    ...
    if (batch.length > 0) await HFAT1_FINAL.insertMany(batch);
    ...
    console.error("Error in HAFT1UploadController:", error);
    res.status(500).send("Internal server error");
  }
}

export const HFAT1GetFinalRows = async (req, res) => {
  try {
    const data = await HFAT1_Mapping.find({})
...
// }
// export const HFAT1UploadController = async (req, res) => {
//   try {
...
//     const result = await HFAT1_FINAL.insertMany(batch, {
//       ordered: false,
//       rawResult: true,
