...
import { HFAT2, HFAT2_FINAL,HFAT2_Mapping } from "../Database/HFAT-2.js";
const app = express();
...
export const HFAT2FINALGet = async (req, res, next) => {
  try {
    const adminId = req.user.id;
    const state = req.user.sitename;
    const role = req.user.role;
    ...
    var HFAT2Data;
    if (role === "superadmin") {
      HFAT2Data = await HFAT2_FINAL.find();
    } else {
      HFAT2Data = await HFAT2_FINAL.find({ uniqueCode: { $regex: regex } });
    }
...
//     let HEAT2Data;
//     if (role === "superadmin") {
//       HEAT2Data = await HFAT2.find();
//     } else {
//       HEAT2Data = await HFAT2.find({ uniqueCode: { $regex: regex } });
//     }
...
export const HFAT2AndAMBULANCEFINALGet = async (req, res, next) => {
  try {
    const id = req.user.id;
    const state = req.user.sitename;
    const role = req.user.role;
    ...
    // Apply conditional logic based on the role
    if (role === "superadmin") {
      HEAT2Data = await HFAT2_FINAL.aggregate([
        ...lookupAmbulanceDetails, // Reuse lookup logic for superadmin
      ]);
    } else {
      HEAT2Data = await HFAT2_FINAL.aggregate([
...
export const HFAT2DownloadCsv = async (req, res) => {
  try {
    ...
    res.setHeader("Content-Disposition", "attachment;filename=HFAT-3.csv");
    res.send(csv);
  } catch (error) {
    console.error(error);
...
export const HFAT2DownloadExcel = async (req, res) => {
  try {
    ...
    res.setHeader("Content-Disposition", 'attachment; filename="HFAT-2.xlsx"');
...
export const HFAT2UpdateController = async (req, res) => {
  try {
    // const { id } = req.params;
    const data = req.body;
    const updatedData = await HFAT2.findByIdAndUpdate(data._id, data);
...
export const HFAT2FinalUpdateController = async (req, res) => {
  try {
    // const { id } = req.params;
    const data = req.body;
    const updatedData = await HFAT2_FINAL.findByIdAndUpdate(data._id, data);
    res.status(200).json({updatedData,succes:true});
  }
  catch (error) {
    console.error(error);
...
export const HFAT2UploadControllerOLD = async (req, res) => {
  try {
    ...
    const mappings = await HFAT2_Mapping.find({}).lean();
    const headerMapping = {};
    mappings.forEach((m) => {
      if (m.headerName && m.field) headerMapping[m.headerName.trim()] = m.field.trim();
    });
    console.log("HFAT-2 Header Mapping Loaded:", headerMapping.length);
    ...
    for await (const row of stream) {
      ...
      if (batch.length >= batchSize) {
        await HFAT2_FINAL.insertMany(batch);
        batch = [];
      }
    }

    // Insert remaining rows
    if (batch.length > 0) await HFAT2_FINAL.insertMany(batch);
    ...
    console.error("Error in HFAT2UploadController:", error);
    res.status(500).send("Internal server error");
  }
}

export const HFAT2GetFinalRows = async (req, res) => {
  try {
    const data = await HFAT2_Mapping.find({}).sort({ index: 1 }).lean();
...
// }
// export const HFAT2UploadController = async (req, res) => {
//   try {
...
//     console.log("HFAT-2 Header Mapping Loaded:", dbHeaderCount);
...
//     const result = await HFAT2_FINAL.insertMany(batch, {
//       ordered: false,
//       rawResult: true,
