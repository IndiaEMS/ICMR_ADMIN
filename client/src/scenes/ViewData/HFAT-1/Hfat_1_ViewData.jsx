...
const Hfat_1_ViewData = () => {
  ...
  const handleDownloadCSV = async () => {
    try {
      ...
      // export to csv
      // set file name
      gridRef.current.api.exportDataAsCsv({ fileName: "HFAT-1.csv" });
    } catch (error) {
      console.log(error);
    } finally {
      setColumnDefs(HFAT1Columns);
    }
  };

  const handleDownload = async () => {
    await fetch(`${url}/HFAT-1/Download`)
  ...
  const downloadExcel = async () => {
    try {
      const response = await axios.get("http://localhost:3000/HFAT-1/Excel", {
        responseType: "blob",
      });
...
export default Hfat_1_ViewData;
